...
from crispy_forms.layout import HTML, Hidden, Layout
...
from core.models import AdresseMail, Mail, Rattachement
...
class Formulaire(FormulaireBase, ModelForm):
    ...
    html = forms.CharField(
        label="Texte",
        widget=SummernoteInplaceWidget(attrs={"summernote": {"width": "100%", "height": "400px"}}),
        required=False,
    )
    documents = forms.CharField(label="Documents", required=False, widget=Documents_joints())
    dest = forms.MultipleChoiceField(
        label="Destinataires",
        required=False,
        widget=Select2TagWidget(
            attrs={
                ...
                "title": "Sélectionnez une adresse dans la liste ou tapez-la directement",
            }
        ),
        choices=[],
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["adresse_exp"].queryset = AdresseMail.objects.filter(
            pk__in=self.request.user.Get_adresses_exp_possibles(), actif=True
        ).order_by("adresse")
        if not self.fields["adresse_exp"].queryset.count():
            self.fields[
            "adresse_exp"
            ].help_text = """<span class='text-danger'><i class='fa fa-warning text-danger'></i> Vous devez vérifier qu'une adresse d'expédition a été créée dans le menu
        ...
        if self.instance:
            ...
            if destinataire.famille:
                for rattachement in Rattachement.objects.select_related("individu").filter(
                    famille=destinataire.famille
                ):
        ...
        self.helper.layout = Layout(
            ...
            Commandes(
                ...
                autres_commandes=[
                    HTML(
                        """<a class="btn btn-primary" id="bouton_envoyer" title="Envoyer"><i class="fa fa-send-o margin-r-5"></i>Envoyer</a> """
                    ),
                    HTML(
                        """<a class="btn btn-danger" title="Fermer" onclick="$('#modal_editeur_emails').modal('hide');"><i class="fa fa-ban margin-r-5"></i>Fermer</a> """
                    ),
                    HTML(
                        """<button type="submit" name="enregistrer_brouillon" title="Enregistrer le brouillon" class="btn btn-default"><i class="fa fa-save margin-r-5"></i>Enregistrer le brouillon</button> """
                    ),
                    HTML(EXTRA_HTML),
                ],
            ),
